...
def combine_datasets_and_shuffle(text_file_path1, text_file_path2, img_width, img_height, base_path, all_images_path, val_ratio=0.2):
    annotations_by_frame = {}
    
    with open(text_file_path1, 'r') as file:
        for line in file:
    ...
    with open(text_file_path2, 'r') as file:
        for line in file:
            parts = line.strip().split(',')
            frame_number, object_id, x, y, width, height, active_status, class_id, additional_info = parts
            class_id = int(class_id) - 1  # Adjusting class id for YOLO (starts from 0)
